...
namespace xe
{
	...
	xeDynamicArray<xeInt64> U8StringRef::FindAll(const xeUnicode separator)
	{
		xeU8cstr utf8_separator[4] = { 0 };
		auto out_size = Utf32ToUtf8(separator, utf8_separator);
		if (out_size == 0)
		{
			XE_WARNING_OUTPUT("<FUNCTION: XESTRING> Not input unicode");
			return xeDynamicArray<xeInt64>();
		}
		if (out_size == 1)return FindAll(utf8_separator[0]);
		else
		{
			return FindAll(utf8_separator, out_size);
		}
	}

	xeDynamicArray<xeInt64> U8StringRef::FindAll(const U8StringRef separator)
	{
		if(separator.length() == 0)
		{
			XE_WARNING_OUTPUT("<FUNCTION: XESTRING> Found string is null");
			return xeDynamicArray<xeInt64>();
		}
		if (separator.length() == 1)
			return FindAll((separator.data())[0]);
		else
		{
			return FindAll(separator.data(), separator.length());
		}
	}

	xeDynamicArray<xeInt64> U8StringRef::FindAll(const xeU8cstr separator)
	{
		return FindByteAllMemoryInt64Size(reinterpret_cast<const xeByte*>(characters_data), length(), separator);
	}

	xeDynamicArray<xeInt64> U8StringRef::FindAll(const xeU8cstr* separator, const xeInt64 size)
	{
		return xeDynamicArray<xeInt64>();
	}

	xeDynamicArray<U8StringRef> U8StringRef::Split(const xeUnicode separator)
	{
		xeDynamicArray<U8StringRef> output;
		return output;
	}

	xeDynamicArray<U8StringRef> U8StringRef::Split(U8StringRef separator)
	{
		return xeDynamicArray<U8StringRef>();
	}
	...
	bool U8StringRef::StringCmp(const xeU8cstr* cmp_str, xeInt64 cmp_str_size)
	{
		if(is_short_string)
		{
			return StringShortCmp(cmp_str, cmp_str_size);
		}
		else
		{
			return StringLongCmp(cmp_str, cmp_str_size);
		}
	}
	...
	bool U8StringRef::StringLongCmp(const xeU8cstr* cmp_str, xeInt64 cmp_str_size)
	{
		if (cmp_str_size != characters_data_size - 1)
			return false;
		return LongMemoryCmp(reinterpret_cast<xeByte*>(characters_data), reinterpret_cast<const xeByte*>(cmp_str), cmp_str_size);
	}

	bool U8StringRef::StringShortCmp(const xeU8cstr* cmp_str, xeInt64 cmp_str_size)
	{
		if (cmp_str_size != characters_data_size - 1)
			return false;
		return ShortMemoryCmp(reinterpret_cast<xeByte*>(characters_data), reinterpret_cast<const xeByte*>(cmp_str), cmp_str_size);
	}
}