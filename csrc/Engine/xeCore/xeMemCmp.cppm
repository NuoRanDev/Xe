...
namespace xe
{
	export bool ShortMemoryCmp(const xeByte* cmp1, const xeByte* cmp2, const xeUint64 cmp_size)
	{
		for (xeUint64 i = 0; i < cmp_size; i++)
		{
	...
	export bool LongMemoryCmp(const xeByte* cmp1, const xeByte* cmp2, const xeUint64 cmp_size)
	{
		xeUint64 i = 0;
	...
	export xeDynamicArray<xeInt64> FindByteAllMemoryInt64Size(const xeByte* str, const xeUint64 size, const xeByte c)
	{
		xeDynamicArray<xeInt64> result;
		if (size == 0 || str)
			return result;
		for (xeInt64 i = 0; i < size; i++)
		{
			if (str[i] == c)
			{
				result.push_back(i);
